<template>
  <div class="conversation-edit-container">
    ...
    <template v-else-if="conversation">
      ...
      <v-card
        class="main-conversation-container"
        variant="elevated"
        color="#fff"
      >
        <!-- Conversation Info -->
              ...
              <v-row class="ma-0 pa-0" align="center">
                <v-col cols="3" class="pa-0">
                  ...
                  >
                    ...
                    <v-list-item-title
                      class="text-body-2 font-weight-medium d-flex align-center"
                ...
                <v-col cols="3" class="pa-0">
                  ...
                  >
                    ...
                    <v-list-item-title
                      class="text-body-2 font-weight-medium d-flex align-center"
                      >Sắp xếp</v-list-item-title
                    >
                    <v-list-item-subtitle
                      class="text-caption d-flex align-center"
                      >Kéo thả để sắp xếp</v-list-item-subtitle
                    >
                  </v-list-item>
                </v-col>
              </v-row>
          ...
          <div>
            ...
            >
              ...
              <div class="text-body-2 text-grey-darken-1">
                Chưa có dòng hội thoại nào. Hãy thêm dòng hội thoại đầu tiên.
              </div>
            </div>
            ...
            >
              <template #item="{ element, index }">
                <div
                  ...
                  :class="
                    element.speaker === 'user' ? 'justify-end' : 'justify-start'
                  "
                  :data-temp-id="element.tempId"
                >
                  <!-- Avatar for Nihongo IT -->
                  <div
                    v-if="element.speaker !== 'user'"
                  ...
                  >
                    ...
                    <div class="message-header px-2 pt-1 d-flex align-center">
                      <div class="line-number me-2 text-caption">
                        #{{ index + 1 }}
                      </div>
                      <v-chip
                        size="x-small"
                        :color="element.speaker === 'user' ? 'primary' : 'info'"
                        variant="tonal"
                      >
                      ...
                      <v-btn
                        ...
                        size="x-small"
                        :color="
                          expandedLines[element.tempId] ? 'primary' : 'default'
                        "
                        variant="text"
                    ...
                    <div
                      v-if="!expandedLines[element.tempId]"
                      class="message-preview px-2 pb-2"
                    >
                      <div
                        class="japanese-text text-body-1 font-weight-medium word-break-wrap"
                      >
                    ...
                    <v-expand-transition>
                      ...
                      >
                        <v-textarea
                          ...
                          :rules="[
                            (v) =>
                              !!v || 'Đoạn hội thoại tiếng Nhật là bắt buộc',
                          ]"
                          required
                          @input="markAsDirty"
                        ></v-textarea>
                        ...
                        <v-textarea
                          ...
                          class="mb-2"
                          :rules="[
                            (v) => !!v || 'Bản dịch tiếng Việt là bắt buộc',
                          ]"
                          required
                          @input="markAsDirty"
                        ></v-textarea>
        ...
        <v-card-text class="pt-4">
          ...
          <div class="mt-2 pa-2 bg-grey-lighten-4 rounded">
            {{
              lineToDelete !== null &&
              conversation?.lines &&
              conversation.lines[lineToDelete]?.japaneseText
            }}
          </div>
        </v-card-text>
        <v-divider></v-divider>
  ...
  try {
    ...
    error.value =
      (err as { response?: { data?: { message?: string } } }).response?.data
        ?.message || "Không thể tải dữ liệu hội thoại. Vui lòng thử lại sau.";
  } finally {
    loading.value = false;
  }
};
...
const saveConversation = async () => {
  ...
  try {
    ...
    if (conversationToSave.lines) {
      conversationToSave.lines.forEach((line: Record<string, unknown>) => {
        delete line.tempId;
      });
    }
    ...
    error.value =
      (err as { response?: { data?: { message?: string } } }).response?.data
        ?.message || "Có lỗi xảy ra khi lưu hội thoại";
...
const addLine = () => {
  ...
  const orderIndex =
    conversation.value.lines.length > 0
...
const moveLineDown = (index: number) => {
  if (
    !conversation.value ||
    !conversation.value.lines ||
    index >= conversation.value.lines.length - 1
  )
    return;
