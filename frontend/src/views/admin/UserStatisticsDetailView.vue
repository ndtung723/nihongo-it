              ...
              <div>
                ...
                <div class="d-flex align-center mt-1">
                  ...
                  <span class="text-caption"
                    >Tham gia:
                    {{ formatDate(userStats.profileInfo?.createdAt) }}</span
                  >
                </div>
                <div class="d-flex align-center mt-1">
                  ...
                  <span class="text-caption"
                    >Đăng nhập gần đây:
                    {{ formatDate(userStats.profileInfo?.lastLogin) }}</span
                  >
                </div>
              </div>
            </v-col>
              ...
              <div class="d-flex flex-column h-100 justify-center">
                ...
                <div class="d-flex justify-space-between mt-2">
                  <span class="text-caption"
                    >Số thẻ đã học:
                    {{
                      (userStats.summary?.totalCards || 0) -
                      (userStats.cardsByState?.new || 0)
                    }}</span
                  >
                  <span class="text-caption"
                    >Tổng thẻ: {{ userStats.summary?.totalCards || 0 }}</span
                  >
                </div>
              </div>
            </v-col>
              ...
              <div class="d-flex flex-wrap justify-center h-100 align-center">
                <div class="text-center mx-3">
                  <div class="text-h5 font-weight-bold">
                    {{ userStats.summary?.currentStreak || 0 }}
                  </div>
                  <div class="text-caption">Chuỗi ngày</div>
                </div>
                <div class="text-center mx-3">
                  <div class="text-h5 font-weight-bold">
                    {{ userStats.summary?.reviewsLast30Days || 0 }}
                  </div>
                  <div class="text-caption">Lần ôn tập</div>
                </div>
                <div class="text-center mx-3">
                  <div class="text-h5 font-weight-bold">
                    {{ formatPercent(userStats.summary?.overallRetentionRate) }}
                  </div>
                  <div class="text-caption">Tỷ lệ ghi nhớ</div>
                </div>
                <div class="text-center mx-3">
                  <div class="text-h5 font-weight-bold">
                    {{ userStats.profileInfo?.currentLevel || "N/A" }}
                  </div>
                  <div class="text-caption">Cấp độ hiện tại</div>
                </div>
                <div class="text-center mx-3">
                  <div class="text-h5 font-weight-bold">
                    {{ userStats.profileInfo?.jlptGoal || "N/A" }}
                  </div>
                ...
                >
                  ...
                  >
                    ...
                    <div class="text-body-1">
                      {{ userStats.cardsByJlptLevel?.[level] || 0 }} thẻ
                    </div>
                    <div class="text-caption">
                      {{
                        calculateJlptPercent(
                          level,
                          userStats.cardsByJlptLevel?.[level] || 0,
                        )
                      }}%
                    </div>
                  </div>
          ...
          >
            ...
            <template v-slot:item.vocabulary="{ item }">
              <div v-if="(item as ReviewHistoryItem).vocabulary">
                <div>
                  {{
                    (item as ReviewHistoryItem).vocabulary?.japanese ||
                    (item as ReviewHistoryItem).vocabulary?.term
                  }}
                </div>
                <div class="text-caption">
                  {{
                    (item as ReviewHistoryItem).vocabulary?.english ||
                    (item as ReviewHistoryItem).vocabulary?.meaning
                  }}
                </div>
                <v-chip size="x-small" color="info" class="mt-1">{{
                  (item as ReviewHistoryItem).vocabulary?.jlptLevel
                }}</v-chip>
              </div>
...
const calculateJlptPercent = (level: string, count: number) => {
  if (
    !userStats.value?.summary?.totalCards ||
    userStats.value.summary.totalCards === 0
  )
    return 0;
...
const fetchUserStatistics = async () => {
  ...
  try {
    userStats.value = await statisticsService.getUserStatisticsById(
      userId.value,
    );
  } catch {
    error.value = true;
