  ...
  <div class="home-page">
    ...
    <section class="hero-section text-center py-16">
      <v-container>
        ...
        <p class="hero-subtitle headline mb-8">
          Nền tảng toàn diện giúp bạn tự tin giao tiếp và làm việc trong ngành
          CNTT tại Nhật Bản.
        </p>
        <v-btn
          color="primary"
          ...
          class="mx-2"
          >Bắt đầu lộ trình</v-btn
        >
        <v-btn color="secondary" large :to="{ name: 'vocabulary' }" class="mx-2"
          >Khám phá từ vựng</v-btn
        >
      </v-container>
    </section>
    ...
    <section v-if="isLoggedIn" class="user-dashboard-summary py-10">
      <v-container>
        <v-row>
          <v-col cols="12">
            <h2 class="section-heading text-h5 font-weight-bold mb-3">
          ...
          <v-col cols="12" md="4">
            <v-card class="pa-4 fill-height d-flex flex-column">
              ...
              <v-card-text class="flex-grow-1">
                ...
                <p>
                  {{ dailyProgress.minutesStudied }} /
                  {{ studyPlan.dailyGoalMinutes }} phút đã học
                </p>
              </v-card-text>
              <v-card-actions>
                <v-btn
          ...
          <v-col cols="12" md="8">
            <v-card class="pa-4 fill-height">
              ...
              <v-card-text v-if="suggestedLessons.length > 0">
                <v-list-item :to="getLessonLink(suggestedLessons[0])">
                  <v-list-item-content>
                    <v-list-item-title class="font-weight-medium">{{
                      suggestedLessons[0].title
                    }}</v-list-item-title>
                    <v-list-item-subtitle>{{
                      suggestedLessons[0].description
                    }}</v-list-item-subtitle>
                  </v-list-item-content>
                  <v-list-item-action>
                ...
                <v-divider
                  class="my-2"
                  v-if="suggestedLessons.length > 1"
                ></v-divider>
                <v-list-item
                  v-if="suggestedLessons.length > 1"
                  :to="getLessonLink(suggestedLessons[1])"
                >
                  <v-list-item-content>
                    <v-list-item-title class="font-weight-medium">{{
                      suggestedLessons[1].title
                    }}</v-list-item-title>
                  </v-list-item-content>
                  <v-list-item-action>
                    <v-icon>mdi-arrow-right</v-icon>
              ...
              <v-card-text v-else>
                <p>
                  Không có bài học nào được đề xuất hôm nay. Hãy khám phá các
                  chủ đề khác!
                </p>
              </v-card-text>
              <v-card-actions>
                <v-btn text color="primary" :to="{ name: 'vocabularyLearning' }"
                  >Xem tất cả bài học</v-btn
                >
              </v-card-actions>
            </v-card>
          </v-col>
    ...
    <section class="features-highlight py-12 bg-grey-lighten-4">
      <v-container>
        <h2 class="section-heading text-h5 font-weight-bold text-center mb-10">
          Công cụ & Tài nguyên Ưu Việt
        </h2>
        <v-row>
          <v-col cols="12" md="4">
            <v-card class="text-center pa-5 fill-height feature-item-card">
              <v-icon size="50" color="primary" class="mb-3"
    ...
    <v-container class="main-content py-10">
      <v-row>
        ...
        <v-col cols="12" md="8">
                ...
                <div class="translation-container">
                  <div class="translation-options pa-3">
                    <v-btn-toggle
                      ...
                      class="w-100"
                    >
                      <v-btn value="vn-to-jp" class="flex-grow-1"
                        >Tiếng Việt → Tiếng Nhật</v-btn
                      >
                      <v-btn value="jp-to-vn" class="flex-grow-1"
                        >Tiếng Nhật → Tiếng Việt</v-btn
                      >
                    </v-btn-toggle>
                  </div>
          ...
          <!-- Recommended Lessons -->
          <h2 class="section-heading text-h5 font-weight-bold mb-4 mt-6">
            Bài học Cho Bạn
          </h2>
          <v-row>
            <v-col
              v-for="lesson in suggestedLessons.slice(0, 2)"
              :key="lesson.id"
              cols="12"
              sm="6"
            >
              <v-card class="lesson-card fill-height d-flex flex-column">
                <v-card-title
                  class="lesson-title text-subtitle-1 font-weight-medium"
                  >{{ lesson.title }}</v-card-title
                >
                <v-card-subtitle>
                  <v-chip
                    size="small"
                    :color="getLessonChipColor(lesson.type)"
                    class="me-2"
                    >{{ getLessonTypeText(lesson.type) }}</v-chip
                  >
          ...
          <div class="text-center mt-4" v-if="suggestedLessons.length > 2">
            <v-btn color="secondary" :to="{ name: 'vocabularyLearning' }"
              >Xem tất cả bài học</v-btn
            >
          </div>
        </v-col>
        ...
        <v-col cols="12" md="4">
          ...
          <h3 class="section-heading text-h6 font-weight-medium mb-3">
            Từ vựng Mới
          </h3>
          <v-card variant="outlined" class="recent-vocab-card mb-6">
            <v-list class="recent-vocab-list py-0">
              <v-list-item
                v-for="vocab in recentVocabulary.slice(0, 5)"
                :key="vocab.id"
                :to="{
                  name: 'vocabulary',
                  query: { keyword: vocab.kanji || vocab.hiragana },
                }"
                class="vocab-item"
              >
                ...
                <div class="w-100">
                  <div class="d-flex align-center justify-space-between">
                    <span class="vocab-text font-weight-medium">{{
                  ...
                  <div class="vocab-meaning text-caption">
                    {{ vocab.meaning }}
                  </div>
                </div>
            ...
            <v-card-actions class="justify-center pa-2">
              <v-btn color="primary" variant="text" :to="{ name: 'vocabulary' }"
                >Xem tất cả từ vựng</v-btn
              >
            </v-card-actions>
          </v-card>
          ...
          <v-card class="search-ranking-card">
            <v-list density="compact" class="py-0">
              <v-list-item
                v-for="(term, index) in trendingSearches.slice(0, 5)"
                :key="index"
              ...
              >
                <template v-slot:prepend>
                  <div
                    class="ranking-number-new"
                    :class="index < 3 ? 'top-rank' : ''"
                  >
                    {{ index + 1 }}
                  </div>
                </template>
                <v-list-item-title class="ranking-item-text">{{
                  term.keyword
                }}</v-list-item-title>
                <template v-slot:append>
...
const translationDirection = ref("vn-to-jp");
const translationText = ref("");
const translationSection = ref<HTMLElement | null>(null);

const isLoggedIn = computed(() => !!authStore.user);
const username = computed(() => authStore.user?.fullName || "Người dùng");
const currentLevel = computed(() => authStore.user?.currentLevel || "N/A");

const dailyProgress = reactive({ minutesStudied: 25 });
const studyPlan = reactive({ dailyGoalMinutes: 60 });

const dailyGoalProgress = computed(() => {
  if (!studyPlan.dailyGoalMinutes) return 0;
  return Math.min(
    Math.round(
      (dailyProgress.minutesStudied / studyPlan.dailyGoalMinutes) * 100,
    ),
    100,
  );
});
...
const recentVocabulary: VocabularyItem[] = [
  {
    id: "1",
    kanji: "プログラム",
    hiragana: "ぷろぐらむ",
...
function getLessonLink(lesson: Lesson): {
  name: string;
...
function searchVocabulary() {
  if (searchQuery.value.trim()) {
    router.push({ name: "vocabulary", query: { keyword: searchQuery.value } });
  }
}

function goToTranslationPage() {
  if (!translationText.value.trim()) return;
  router.push({
    name: "translations",
    query: { text: translationText.value, dir: translationDirection.value },
  });
}

function scrollToTranslation() {
  translationSection.value?.scrollIntoView({
    behavior: "smooth",
    block: "center",
  });
}

function getLessonChipColor(type: string): string {
  switch (type) {
    case "vocabulary":
      return "info";
    case "conversation":
...
function getLessonTypeText(type: string): string {
  switch (type) {
    case "vocabulary":
      return "Từ vựng";
    case "conversation":
...
.main-content {
  ...
  .translation-card {
    ...
    .v-btn-toggle .v-btn {
      // Ensure buttons in toggle take full width if needed
...
@media (max-width: 600px) {
  ...
  .hero-section {
    ...
    .v-btn + .v-btn {
      // Add margin between buttons if they stack
