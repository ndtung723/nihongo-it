					...
					<v-row class="mt-3" v-if="hasActiveFilters">
						<v-col cols="12">
							<div class="d-flex flex-wrap align-center">
								...
								<v-btn size="small" variant="text" density="comfortable" @click="clearAllFilters">
									Xóa tất cả
								</v-btn>
							</div>
						</v-col>
					...
					<div class="pa-3">
						<div class="d-flex align-center justify-space-between">
							...
							<v-btn
								...
								:disabled="
									(playingAudioId !== null && playingAudioId !== item.vocabId) ||
							...
							<div v-if="item.example" class="example-section mb-2 pa-2 rounded">
								<div class="d-flex align-items-center">
									...
									<v-btn
										...
										:disabled="
											(playingExampleAudioId !== null && playingExampleAudioId !== item.vocabId) ||
											playingAudioId !== null
										"
									></v-btn>
								</div>
					...
					<div v-if="chatGPTItems.includes(item.vocabId)" class="pa-3 pt-0" @click.stop>
						<div class="chatgpt-content py-2 px-2 rounded" @click.stop>
							...
							<template v-if="item.chatHistory && item.chatHistory.length > 0">
								<div v-for="(message, msgIndex) in item.chatHistory" :key="msgIndex" class="mb-2">
									<!-- User Message -->
...
const animateTypingChatResponse = (
	vocabId: string,
	text: string,
	vocabIndex: number,
	messageIndex: number,
	...
	const typeNextChar = () => {
		if (currentIndex < text.length) {
			if (
				...
				vocabulary.value[vocabIndex].chatHistory![messageIndex].content = text.substring(
					0,
					currentIndex + 1,
				);
			}
