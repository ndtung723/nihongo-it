<template>
  <v-container fluid class="vocabulary-container pa-0">
    <!-- Header Section -->
    <div
      class="header px-4 py-3 d-flex justify-space-between align-center mb-2"
    >
    ...
    <div v-else class="vocabulary-content px-4">
      <div v-if="vocabulary.length === 0" class="text-center pa-6">
        ...
        <v-btn
          ...
          @click="router.push('/admin/vocabulary')"
        >
          Thêm từ vựng
        </v-btn>
      </div>
          ...
          <div class="pa-3">
            <div class="d-flex align-center justify-space-between">
              <div class="item-text">
                <div class="term-container">
                  ...
                  <div
                    ...
                    class="japanese-text d-none d-md-inline-block ml-2"
                  >
                    ({{ item.pronunciation }})
                  </div>
                  <v-chip
              ...
              <v-btn
                ...
                :disabled="
                  (playingAudioId !== null &&
                    playingAudioId !== item.vocabId) ||
                  playingExampleAudioId !== null
                "
              ></v-btn>
              ...
              >
                <div class="d-flex align-items-center">
                  ...
                  <v-btn
                    ...
                    :disabled="
                      (playingExampleAudioId !== null &&
                        playingExampleAudioId !== item.vocabId) ||
                      playingAudioId !== null
                    "
                  ></v-btn>
                </div>
              ...
              <div class="additional-info d-flex flex-wrap mt-2">
                <div class="mr-4 mb-1">
                  ...
                  <span class="text-caption ml-1">{{
                    formatDate(item.createdAt ?? "1999")
                  }}</span>
                </div>
              </div>
            </div>
...
const hasActiveFilters = computed(() => {
  return !!(filters.keyword || filters.jlptLevel || filters.topicName);
});
...
const fetchTopics = async () => {
  try {
    topics.value = (await vocabularyService.getTopics()) as {
      id: string;
      name: string;
    }[];
  } catch {}
...
const playAudio = async (
  ...
  isExampleSentence: boolean = false,
): Promise<void> => {
  if (!vocabItem) {
  ...
  if (index >= 0) {
    ...
    api
      ...
      .then((response) => {
        ...
        const vocabIndex = vocabulary.value.findIndex(
...
const animateTyping = (vocabId: string, text: string) => {
  ...
  const typeNextChar = () => {
    if (currentIndex < text.length) {
      ...
      const vocabItem = vocabulary.value.find(
        (item) => item.vocabId === vocabId,
      );
...
const animateExamples = (vocabId: string, examples: unknown[]) => {
  if (
    !examples ||
    examples.length === 0 ||
...
const animateExampleText = (
  ...
  if (vocabIndex !== -1 && vocabulary.value[vocabIndex].aiExamples) {
    const example = vocabulary.value[vocabIndex].aiExamples[
      exampleIndex
    ] as unknown;
...
const sendChatMessage = async (vocabId: string) => {
  ...
  try {
    ...
    await aiService.streamVocabularyChat(
      ...
      message,
      (chunk) => {
        if (vocabulary.value[vocabIndex]?.chatHistory?.[aiMessageIndex]) {
          vocabulary.value[vocabIndex].chatHistory![aiMessageIndex].content +=
            chunk;
        }
      },
      () => {
    ...
    vocabulary.value[vocabIndex].chatHistory![aiMessageIndex] = {
      role: "assistant",
      content:
...
const isJapaneseComplete = (example: AnimatedExample): boolean => {
  return !!(
    example &&
...
const hasVietnameseStarted = (example: AnimatedExample): boolean => {
