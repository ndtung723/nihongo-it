	...
	<div class="flashcard-study-container">
		...
		<v-container v-else-if="dueCards.length === 0 && !loading" class="text-center py-6">
			<v-card class="pa-6 mx-auto empty-state-card" max-width="500">
				...
				<div class="d-flex justify-center">
					...
					<v-btn color="primary" size="small" @click="refreshDueCards"> Tải Lại </v-btn>
				</div>
			</v-card>
		</v-container>
		...
		<v-container v-else-if="currentCard" class="py-4">
			<div class="mb-3 d-flex align-center progress-container px-2">
				<div class="text-caption text-medium-emphasis">{{ remainingCount }} thẻ còn lại</div>
				<v-spacer></v-spacer>
				<v-chip :color="getStateColor(currentCard.state)" size="x-small" class="state-chip">
		...
		<v-dialog v-model="showStats" max-width="400">
			<v-card>
				<v-card-title class="text-subtitle-1 pt-4 pb-2"> Thống Kê Học Tập </v-card-title>
				...
				<v-card-actions>
					...
					<v-spacer></v-spacer>
					<v-btn color="grey" variant="text" @click="showStats = false" size="small"> Đóng </v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
	</div>
...
async function playAudio(card: FlashcardDTO) {
	...
	try {
		...
		try {
			const response = await api.get(`/api/v1/speech/vocabulary/${card.vocabularyId}/audio`, {
				responseType: 'blob',
			});
...
async function playExampleAudio(card: FlashcardDTO) {
	...
	try {
		...
		const response = await api.post('/api/v1/ai/tts/generate', exampleSentence, {
			headers: {
				'Content-Type': 'text/plain; charset=UTF-8',
				'Accept-Language': 'ja-JP',
				'X-Speech-Speed': '0.9',
