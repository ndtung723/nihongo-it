<template>
	<div class="vocabulary-topic-container">
						...
						<div class="d-flex align-center justify-space-between">
							<div class="item-text">
								<div class="term-container">
									...
									<v-chip :color="getJlptColor(item.jlptLevel)" size="x-small" class="ml-2">
										{{ item.jlptLevel }}
									</v-chip>
								</div>
												...
												>
													...
													<template v-else-if="typingExamples[item.vocabId] === exIndex">
														...
														<p class="example-text text-caption">
															...
															<span
																v-if="isJapaneseComplete(example) && !hasVietnameseStarted(example)"
																class="typing-cursor"
																>|</span
															>
														</p>
		...
		<div v-else class="text-center py-8">
			...
			<p class="text-body-1 text-medium-emphasis">The requested topic could not be found.</p>
			<v-btn color="primary" class="mt-4" @click="goToCategories"> View All Categories </v-btn>
		</div>

		<!-- Filter Dialog -->
		<v-dialog v-model="showFilterDialog" max-width="400">
			<v-card>
				<v-card-title class="text-h6 font-weight-bold"> Lọc từ vựng </v-card-title>
				<v-card-text>
					<v-select
		...
		if (Array.isArray(topicsResponse)) {
			...
			if (!foundTopic) {
				foundTopic = topicsResponse.find(
					(t) => t.name === decodedParam || encodeURIComponent(t.name) === paramValue,
				);
			}
...
async function sendChatMessage(vocabId: string) {
	...
	try {
		...
		const response = await api.post('/api/v1/ai/chat/vocabulary/chat', null, {
			params: {
				vocabWord: vocabWord,
				userMessage: message,
			},
...
function animateTypingChatResponse(
	vocabId: string,
	text: string,
	vocabIndex: number,
	messageIndex: number,
	...
	const typeNextChar = () => {
		if (currentIndex < text.length) {
			if (
				...
				vocabularyItems.value[vocabIndex].chatHistory![messageIndex].content = text.substring(
					0,
					currentIndex + 1,
				);
			}
