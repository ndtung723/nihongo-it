...
class StatisticsService {
	...
	async getAllUserStatistics(
		...
		try {
			const response = await api.get('/api/v1/learning/admin/statistics/users', {
				params: {
					page,
					size,
					sortBy,
	...
	async getUserStatisticsById(userId: string): Promise<UserStatisticsDetail> {
		try {
			const response = await api.get(`/api/v1/learning/admin/statistics/users/${userId}`);
	...
	async getStatisticsOverview(): Promise<AdminStatisticsOverview> {
		try {
			const response = await api.get('/api/v1/learning/admin/statistics/overview');
			return response.data.data;
		} catch (error) {
			throw error;
		}
