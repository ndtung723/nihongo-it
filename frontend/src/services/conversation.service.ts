...
const conversationService = {
	...
	getConversationsByJlptLevel(
		...
		return api.get(`/api/v1/learning/conversations/jlpt/${level}?${params.toString()}`);
	},
	...
	adminGetConversationsByJlptLevel(
		...
		return api.get(`/api/v1/learning/admin/conversations/jlpt/${level}?${params.toString()}`);
	},
	...
	unsaveConversation(conversationId: string): Promise<AxiosResponse<any>> {
		return api.delete(`/api/v1/user/user/saved-conversations/${conversationId}`);
	},

	// Kiểm tra hội thoại đã được lưu hay chưa
	checkSavedConversation(conversationId: string): Promise<AxiosResponse<{ saved: boolean }>> {
		return api.get(`/api/v1/user/user/saved-conversations/check/${conversationId}`);
	},
	...
	getSavedConversations(
		...
		return api.get(`/api/v1/user/user/saved-conversations?${params.toString()}`);
	},
};
