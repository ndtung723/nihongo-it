...
# This script starts all the Spring Boot services in the project without Docker (excluding AI Service)
...
echo -e "${BLUE}=== Nihongo-IT - Spring Boot Services Starter (without AI Service) ===${NC}"
...
echo "$EUREKA_PID $API_GATEWAY_PID $USER_SERVICE_PID $LEARNING_SERVICE_PID $NOTIFICATION_SERVICE_PID" > "$PROJECT_ROOT/logs/spring_pids.txt"
...
echo -e "${YELLOW}PIDs: Eureka=$EUREKA_PID, API Gateway=$API_GATEWAY_PID, User Service=$USER_SERVICE_PID, Learning Service=$LEARNING_SERVICE_PID, Notification Service=$NOTIFICATION_SERVICE_PID${NC}"
...
while true; do
    ...
    for pid in $EUREKA_PID $API_GATEWAY_PID $USER_SERVICE_PID $LEARNING_SERVICE_PID $NOTIFICATION_SERVICE_PID; do
